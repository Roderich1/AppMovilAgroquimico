...
| RISK-017 | Audio/transcripción sensible sale o queda almacenado | M | H | **Mitigado en `EVO-009`**: el audio no cruza a Dart, no se persiste nada, no se declara `INTERNET` y los diagnósticos llevan códigos y longitudes, nunca frases. Verificado por guardas que fallan si el subsistema toca disco o red (`test/voice/voice_architecture_guard_test.dart`) | Security |
...
| RISK-023 | `es-BO` no existe como idioma de reconocimiento y el modo offline depende de un paquete de idioma ya instalado | H | H | **Mitigado en `EVO-009`**: `VoiceLocalePolicy` recorre una lista documentada de diez candidatos españoles, empezando por `es-BO`, y avanza sólo ante `localeUnavailable`. La pantalla muestra siempre idioma **solicitado** y **utilizado**, y avisa cuando difieren. `es-US` no está fijado en el código. El modo sin conexión se afirma **sólo** tras transcribir con el modo avión activo; `isOnDeviceRecognitionAvailable()` se muestra como dato crudo del sistema, nunca como promesa. Cubierto por `test/voice/voice_locale_policy_test.dart` y las pruebas de pantalla. **Ampliado con el HONOR JDY-LX3P (API 36)**: allí el sistema **declara** `es-US` y `es-ES` y ambos fallan con error 13 porque el modelo no está descargado; ningún español del aparato sirve y la pantalla lo dice sin descargar nada. El locale solicitado pasó a `es-US` por decisión del propietario (revisión de `ADR-002`). Riesgo residual: la lista puede no contener el español de un OEM concreto, y un teléfono sin paquete de voz deja la función inutilizable aunque la app no se bloquee | Voice/Product |
...
| RISK-029 | La continuidad reabre el micrófono en bucle ante `ERROR_CLIENT`, `noMatch` o timeout, y lo deja escuchando o agota la batería | M | H | `VoiceContinuityPolicy`: máximo de turnos improductivos seguidos, backoff acotado que se duplica hasta un tope, tope absoluto de turnos y duración máxima de sesión. Un turno productivo pone el contador a cero; permiso denegado, cancelación, error fatal, segundo plano y salida de pantalla **no** reabren nunca. Cubierto con `fake_async` en `test/voice/voice_session_controller_test.dart` | Voice/Mobile QA |
| RISK-030 | Un segmento nuevo del motor sobrescribe una corrección manual del usuario | M | M | Regla única de composición: el texto de sesión es del usuario y lo dictado **se añade al final**, nunca reescribe. El parcial vive separado y no entra hasta que el motor lo confirma. Deshacer se desactiva en cuanto el usuario edita. Probado en la sesión y en la pantalla | Voice/Product |
...
`RISK-029` y `RISK-030` aparecen con la implementación de `EVO-009`: no son
hipótesis del diseño documental sino los dos modos de fallo que la continuidad
controlada y la edición manual introducen, y por eso nacen ya con prueba
automatizada. Su cierre depende de la verificación en dispositivo físico.
