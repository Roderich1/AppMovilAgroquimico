...
> declarados en §7. `EVOLUTION-2_FINAL_VERIFICATION.md` no existe todavía, y no debe crearse
> hasta que esos gates estén cubiertos con evidencia real.
...
| Java local | Temurin **21.0.10** (el CI usa 17; ver §7) |
...
| Gate | Estado | Motivo |
|---|---|---|
| CI de GitHub Actions sobre el SHA final | **PENDIENTE** | requiere publicar la rama y que el workflow se ejecute |
| Verificación en Pixel 8 / Android 16 (API 36) | **PENDIENTE** | no hay dispositivo ni emulador disponible en este entorno |
| Java 17 | **NO REPRODUCIDO EN LOCAL** | el entorno local tiene Temurin 21; el CI sí usa 17, y ése es el gate que cuenta |
...
| Comportamiento real del filesystem de Android | **pendiente** de la prueba de dispositivo |
| Reportes muy grandes | probado con 250 filas en CSV y 200 en PDF; no se ha medido con miles |

## 9. Fuera de alcance — confirmación explícita
