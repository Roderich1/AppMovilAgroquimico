...
| EVO-009 | Voz: captura/transcripción/preview | EVOLUTION-3 | IN_PROGRESS | P1 | H | PR #8 abierta y **congelada**. El gate físico en HONOR API 36 encontró que ese aparato no tiene ningún español de reconocimiento (`RISK-029`), lo que abrió la Fase 0-bis y `ADR-004` (`Proposed`) |
...
2. Ejecutar benchmark y aceptar/rechazar candidatos en `ADR-002`.
   **Completado.** El propietario ejecutó el corpus de ajuste con los tres motores en un POCO
   X5 Pro 5G (Android 12 / API 31), modo avión verificado contra el sistema. `ADR-002` está
   **`Accepted`**: motor productivo = Android `SpeechRecognizer`; Whisper queda como reserva no
   distribuida. Gate Pixel 8 / API 36 `WAIVED_BY_OWNER` (`RISK-028`). Ver
...
2-bis. **Fase 0-bis: benchmark del motor híbrido local.** Abierta por `RISK-029`. Evalúa Vosk
   (parciales) + Whisper small-q5_1 (final) frente a Android y a Whisper base, en POCO API 31 y
   HONOR API 36, sobre el banco de `benchmark/voice_benchmark` y **sin tocar `lib/`**. Vive en
   la rama `evolution/evolution-3-hybrid-voice-benchmark` y su propia PR. `ADR-004` sigue
   `Proposed` hasta que el propietario decida con las métricas delante.
3. Implementar `EVO-009` en una rama/PR propia. **En curso**: PR #8, congelada mientras dure la
   Fase 0-bis. Si `ADR-004` se acepta, se sustituye su adaptador según
   `features/EVO-009_HYBRID_ENGINE_MIGRATION_PLAN.md`; si se rechaza, la PR continúa como
   está.
